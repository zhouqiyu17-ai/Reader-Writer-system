...
int should_exit = 0; // Flag to signal threads to exit
...
void* reader(void* arg) {
    ...
    while (1) {
        ...
        while (writer_active && !should_exit) {
...
void* writer(void* arg) {
    ...
    while (1) {
        pthread_mutex_lock(&mutex);
        while ((writer_active || read_count > 0) && !should_exit) {
            pthread_cond_wait(&cond_var, &mutex);
        }
        if (should_exit) {
            pthread_mutex_unlock(&mutex);
...
int main() {
    ...
    pthread_mutex_lock(&mutex);
    should_exit = 1;
    pthread_cond_broadcast(&cond_var); // Wake all waiting threads
    pthread_mutex_unlock(&mutex);
